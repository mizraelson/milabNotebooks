...
def minnn_run(R1, R2, minnn_output, fastq_output):
    sample_name = re.sub("(.*/)(.*)(_R[1,2].*\.gz)", r"\2", R1)
    print(sample_name)
...
def merge_clonesets(samples_list, names_list, on="CDR3nt", value="count", how="outer"):
    ...
    samples = [c.groupby(on).sum()[value] for c in samples]
